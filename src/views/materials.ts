    ...
    <section class="workspace workspace--materials">
      <aside class="collection-pane" id="collectionPane">
        <div class="collection-tools">
          ...
          <select id="categoryFilter" class="compact-select" aria-label="文献分类">
        ...
        <nav class="collection-list" aria-label="文献目录">
          ${visible.map((material) => `
            <a class="collection-item ${material.id === options.selected.id ? "is-active" : ""}" href="#/materials/${encodeURIComponent(material.id)}" data-material-id="${material.id}">
              <span class="collection-item__title">${escapeHtml(material.title)}${material.status ? `<small class="material-status">${escapeHtml(material.status)}</small>` : ""}</span>
      ...
      <article class="reader-pane">
        <header class="reader-header">
          <div class="reader-heading">
            ...
            <div>
              <div class="eyebrow">${escapeHtml(options.selected.category)} · ${escapeHtml(options.selected.format.toUpperCase())}${options.selected.status ? ` · ${escapeHtml(options.selected.status)}` : ""}</div>
